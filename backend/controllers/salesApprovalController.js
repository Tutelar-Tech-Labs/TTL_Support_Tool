...
const initTable = async () => {
  try {
    ...
    // console.log("[Sales] Starting schema patch for Stages 2-7...");
    for (const update of schemaUpdates) {
      // Check columns once per table to minimize queries
      const [existingCols] = await db.query(`SHOW COLUMNS FROM ${update.table}`);
      const colNames = existingCols.map(c => c.Field.toLowerCase());

      for (const colDef of update.cols) {
        const colNameMatch = colDef.match(/ADD COLUMN IF NOT EXISTS (\w+)/i);
        if (colNameMatch) {
          const colName = colNameMatch[1].toLowerCase();
          if (colNames.includes(colName)) continue; // Skip if exists
        }

        try {
          await db.query(`ALTER TABLE ${update.table} ${colDef}`);
        } catch (e) {
          // Fallback for older MySQL versions or other issues
          if (!e.message.includes("Duplicate column name")) {
            console.warn(`[Sales] Failed to add column to ${update.table}:`, e.message);
          }
        }
      }
    }

    console.log("[Sales] Approvals table and stage schemas verified");
  } catch (err) {
    if (err.code === 'ETIMEDOUT') {
      console.error("[Sales] Connection timeout during table initialization. System will retry on next request.");
    } else {
      console.error("[Sales] Error during table initialization:", err.message);
    }
  }
};
