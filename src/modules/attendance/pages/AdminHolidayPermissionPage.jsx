...
const AdminHolidayPermissionPage = () => {
  ...
  const loadRequests = async () => {
    try {
      const response = await holidayPermissionAPI.getAllPermissions();
      setRequests(response.data.permissions);
    } catch (error) {
  ...
  const handleReview = async (status) => {
    ...
    try {
      await holidayPermissionAPI.reviewPermission(selectedRequest._id, { status, adminComment });
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-dark-900 dark:text-white">Holiday Work Approvals</h1>
        <p className="text-dark-600 dark:text-slate-400">Review and manage employee requests to work on holidays</p>
      </div>
              ...
              {requests.map((request) => (
                <tr key={request._id} className="hover:bg-slate-50 dark:hover:bg-servicenow-dark/50 transition-colors">
                  ...
                  <td className="py-4 text-dark-600 dark:text-slate-300">
                    {format(new Date(request.holidayDate), 'MMM dd, yyyy')}
                  </td>
                  <td className="py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-servicenow-light rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="p-6 border-b border-slate-100 dark:border-slate-800">
              <h2 className="text-xl font-bold text-dark-900 dark:text-white">Review Permission Request</h2>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                <div className="text-sm text-dark-500 dark:text-slate-400">Holiday Date</div>
                <div className="font-semibold text-dark-900 dark:text-white">
                  {format(new Date(selectedRequest.holidayDate), 'EEEE, MMMM do, yyyy')}
                </div>
              </div>
              <div className="space-y-2">
...
export default AdminHolidayPermissionPage;
