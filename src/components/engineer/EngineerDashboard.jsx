...
export default function EngineerDashboard() {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/tickets/dashboard`);
    ...
    const fetchApprovals = async () => {
      ...
      if (!userId) return;
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/approvals/my/${userId}`);
        const data = await response.json();
        const map = {};
